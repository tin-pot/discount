...
- Added pre-configured files
  ...
  in the directory `MSVC/` so that no `configure.sh` run---which
  generates these files on _UNIX_---is needed on _Windows_.

- Added _Visual Studio_ project files for all executables and libraries
  in `MSVC/`.
...
- Some new features are also available, see the
  [CHANGES](CHANGES_tin-pot.html) file for more details.


2  Building/Installing
----------------------

The package should build "out of the box" with _Visual Studio 2003.NET_
or _Visual Studio 2008_ or newer (newer versions of _Visual Studio_ will
import and upgrade the project files).
...
Porting to _Windows_ was done using a fairly general technique, which
presents to the compiler an environment of "faked" standard header files
on top of the "real" header files of _Visual C++_. All this is contained
in the `MSVC/posc` directory. See the [`MSVC/posc/README`][posc] file
there for explanations.

All `discount` source files in the project root directory are unmodified
(except for new features, that is). Their dependency on functions like
`strncasecmp()` (_UNIX_), or `vsnprintf()` (C99), or `getopt()` gets
satisfied completely out of files in `posc`.
